...
contract ExamPool {
    ...
    function createExam(string memory _description, string memory _typeOfWork, uint8 _typeOfSubmission,
        string memory _subject, uint _submissionTime, address [] memory _student, address [] memory _professor)
    public restrictedToAdmin {
        require(profs[_professor[0]] && students[_student[0]]);
        ExamSubmission newExamAddress = new ExamSubmission(_description, _typeOfWork, _typeOfSubmission, _subject,
            _submissionTime, _student, _professor);
...
contract ExamSubmission {
    ...
    //  constructor
    constructor(string memory _description, string memory _typeOfWork, uint8 _typeOfSubmission,
        string memory _subject, uint _submissionTime, address[] memory _student, address[] memory _professor) public {
    ...
    function createExam(string memory _description, string memory _typeOfWork, uint8 _typeOfSubmission,
        string memory _subject, uint _submissionTime, address[] memory _student, address[] memory _prof) private {
    ...
    function getDetailsOfExam() public view returns (string memory, string memory, string memory, uint, uint, uint,
        string memory, address[] memory, address[] memory, ExamStatus){
        return (
        exam.description,
        exam.subject,
        exam.typeOfWork,
    ...
    */
    ...
    function submitExam(string memory _name, string memory _input) restrictedToStudent public {
        ...
        require(newExam.status == ExamStatus.ToSubmit ||
            (newExam.status != ExamStatus.InCorrection && newExam.status != ExamStatus.Corrected));
